...
export const CONTRACT_ADDRESS =
  "CDEIC6OUQ7SOEML2YY5G5YW5ZJZSTJ7KTRP7F5UTKYUS6RCHMAUCSB5W";
...
// ============================================================
// Simple DEX — Contract Methods
// ============================================================

/**
 * Deposit tokens into the DEX.
 * Calls: deposit(user: Address, token: Symbol, amount: i128)
 */
 ...
 * Calls: withdraw(user: Address, token: Symbol, amount: i128)
 */
 ...
 * Calls: swap(user: Address, token_in: Symbol, token_out: Symbol, amount: i128, rate: i128)
 */
export async function swap(
  ...
  return callContract(
    ...
    [
      toScValAddress(caller),
      toScValSymbol(tokenIn),
      toScValSymbol(tokenOut),
      toScValI128(amount),
      toScValI128(rate),
    ],
    caller,
...
/**
 * Get token balance for a user (read-only).
 * Calls: get_balance(user: Address, token: Symbol) -> i128
 */
export async function getBalance(
  ...
  return readContract(
    "get_balance",
    [toScValAddress(user), toScValSymbol(token)],
    caller
  );
}
